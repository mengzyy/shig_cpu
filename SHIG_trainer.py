...
class SHIGNetwork(torch.nn.Module):
    ...
    def setup_layers(self):
        ...
        self.aggregator = SHIG_Model(self.X.shape[1], self.neurons[-1], num_layers=self.args.num_layers,
                                     trial=self.trial, args=self.args)
...
class SHIGTrainer(object):
    ...
    def setup_dataset(self):
        ...
        self.positive_edges = torch.from_numpy(np.array(self.positive_edges,
                                                        dtype=np.int64).T).type(torch.long)
    ...
    def score_model(self, epoch, last=False):
        ...
        """
        self.model.eval()
        score_positive_edges = torch.from_numpy(np.array(self.test_positive_edges, dtype=np.int64).T).type(torch.long)
        score_negative_edges = torch.from_numpy(np.array(self.test_negative_edges, dtype=np.int64).T).type(torch.long)
